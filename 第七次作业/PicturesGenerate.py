...
# 读取数据
df = pd.read_csv("model_results/tabnet_subject_ranking_model_results.csv")
...
for i, (bar, value) in enumerate(zip(bars1, sorted_df_r2["r2"])):
    plt.text(value + 0.005, i, f"{value:.5f}", va="center", fontsize=8)
...
# 4. MSE 对比条形图
plt.figure(figsize=(12, 8))
sorted_df_mse = df.sort_values("mse", ascending=False)
bars4 = plt.bar(sorted_df_mse["subject"], sorted_df_mse["mse"], color="lightgreen")
plt.ylabel("MSE")
plt.title("各学科 MSE 对比")
plt.xticks(rotation=90)
plt.grid(axis="y", alpha=0.3)

# 添加数值标签
for i, (bar, value) in enumerate(zip(bars4, sorted_df_mse["mse"])):
    plt.text(
        i, value + max(sorted_df_mse["mse"]) * 0.01, f"{value:.1f}", ha="center", va="bottom", fontsize=8
    )

plt.tight_layout()
plt.savefig("pictures/mse_comparison.png", dpi=300, bbox_inches="tight")
plt.show()
plt.close()
