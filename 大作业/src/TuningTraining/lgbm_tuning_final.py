...
def objective(trial, X_train, y_train, X_test, y_test):
    ...
    # LightGBM 的早期停止通过 callbacks 实现
    model.fit(
        X_train, y_train, eval_set=[(X_test, y_test)], callbacks=[lgb.early_stopping(stopping_rounds=100)]
    )
...
def generate_assets(y_test, y_prob, model, params):
    ...
    ax1.plot(fpr, tpr, label=f"Tuned LGBM (AUC={auc:.4f})", color="green", lw=2)
    ax1.plot([0, 1], [0, 1], "k--", alpha=0.3)

    try:
        axins = inset_axes(ax1, width="40%", height="40%", loc="lower right", borderpad=3)
        axins.plot(fpr, tpr, color="green", lw=2)
    ...
    ax2.plot(rec, prec, color="darkgreen", lw=2, label="PR Curve")
    ax2.set_title("PR Curve (LGBM Tuned)")
    ax1.set_title("ROC Curve (LGBM Tuned)")
    ax1.legend()
...
def main():
    ...
    print(f"\n✨ 所有 LightGBM 调参结果已归档至: {Config.RESULT_DIR}")
